...
void Crawler::getLinksParallel(const int& num_t) {
	num_crawled++;								/* increase when crawler start executing crawling a url*/
	string contents = getContents(URL);			/* get all contain from target URL */
	if (contents.size() != 0) {
		int binSize = contents.size() / num_t;	    /* devide chunk size of parallel parsing*/
		#pragma omp parallel for num_threads(num_t)
		for (int i = 0; i < num_t; i++) {
			getLinks(contents, binSize, i);
		}
	}
}
...
void Crawler::getLinks(const string& inputs, const int& binSize, const int& i)
{
	int tid = omp_get_thread_num();
	int start = i * binSize;
	string contents = inputs.substr(i, binSize);	/* divided contain into piecese */
	cout << " omp thread " << tid << " parsing from " << start << endl;
	int t = 3000;
	int next = 0;
	while (t--)
	{
		...
		/* collect links into local links vector
...
/* store links of extract by crawler into local link queue
   verify links with visited set and push links into crawler's queue */
